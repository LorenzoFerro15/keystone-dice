...
int mbedtls_pk_write_pubkey_der(const mbedtls_pk_context *key, unsigned char *buf, size_t size)
{
   ...
   // *--c = 0;
    ...
    //const char oid[] = {0x2B, 0x65, 0x70};
    //const char oid[]= {"\x2a" "\x86\x48" "\xce\x3d" "\x02\01"};
    //oid_len = sizeof("\x2a" "\x86\x48" "\xce\x3d" "\x02\01") - 1;
   // const char oid[]= {"\x2a" "\x86\x48" "\xce\x3d" "\x04" "\x03" "\x04"};
    //oid_len = sizeof("\x2a" "\x86\x48" "\xce\x3d" "\x04" "\x03" "\x04") - 1;
...
int mbedtls_x509_string_to_names_mod(mbedtls_asn1_named_data *head, const char *name, int *ne)
{
    int ret = 0;
    const char *s = name, *c = s;
    const char *end = s + strlen(s);//my_strlen(s);
    const char *oid = NULL;
    const x509_attr_descriptor_t *attr_descr = NULL;
    int in_tag = 1;
    char data[MBEDTLS_X509_MAX_DN_NAME_SIZE];
    ...
    while (c <= end) {
        ...
        if (!in_tag && *c == '\\' && c != end) {
            ...
            int pos=
                mbedtls_asn1_store_named_data_mod(head, oid, strlen(oid),//my_strlen(oid),
                                              (unsigned char *) data,
                                              d - data, ne);
            /*
            if (cur == NULL) {
                            ...
                            void *p_ctx)
{
    ...
    while (*p < end) {
        ...
        *p += extn_oid.len;
        ...
        if(memcmp(extn_oid.p, MBEDTLS_OID_KEY_USAGE, 3) == 0 ){
                                      ...
                                      dice_tcbInfo* info_struct)
{
   ...
   if ((ret = mbedtls_asn1_get_bitstring_null(p, end, &len)) != 0) { //funzione che estrae la chiave pubblica
    ...
    memcpy(info_struct->flags, *p, len);    
    *p += len;
    ...
    *p += app_len;
    ...
    memcpy(info_struct->flagMask, *p, len);    
    *p += len;
